...
public class THFPage extends Page 
  ...
  */
   ...
   * @exception IOException I/O errors
   * in C++ Status insertQuadruple(char *recPtr, int recLen, QID& qid)
   */
  public QID insertQuadruple ( byte [] quadruple)		
	...
	System.arraycopy (quadruple, 0, data, usedPtr, recLen);
	curPage.pid = Convert.getIntValue (CUR_PAGE, data);
   ...
   * @exception IOException I/O errors
   * in C++ Status deleteQuadruple(const QID& qid)
   */
  ...
  /**
   * @return QID of first quadruple on page, null if page contains no records.  
   * @exception  IOException I/O errors
   * in C++ Status firstQuadruple(QID& firstRid)
   * 
   */ 
   ...
   * @param 	curRid	current quadruple ID
   * @exception  IOException I/O errors
   * in C++ Status nextQuadruple (QID curRid, QID& nextRid)
   */
  public QID nextQuadruple (QID curRid) 
    throws IOException 
    {
      QID qid = new QID();
      slotCnt = Convert.getShortValue (SLOT_CNT, data);
   ...
   * <br>
   * Status getQuadruple(QID qid, char *recPtr, int& recLen)
   * @param	qid 	the quadruple ID
   * @return 	a quadruple contains the quadruple
   * @exception   InvalidSlotNumberException Invalid slot number
   * @exception  	IOException I/O errors
   * @see 	Quadruple
   */
  public Quadruple getQuadruple ( QID qid ) 
	  ...
	  && (pageNo.pid == curPage.pid))
	{
	  ...
	  Quadruple aquadruple = new Quadruple(quadruple, 0); // recLen - is always 32 for Quadruple 
	  return aquadruple;
	}
  ...
  /**
   * returns a quadruple in a byte array[pageSize] with given QID qid.
   * <br>
   * in C++	Status returnQuadruple(QID qid, char*& recPtr, int& recLen)
   * @param       qid     the quadruple ID
   * @return      a quadruple  with its length and offset in the byte array
   * @exception   InvalidSlotNumberException Invalid slot number
   * @exception   IOException I/O errors
   * @see 	Quadruple
   */  
  public Quadruple returnQuadruple ( QID qid )
	  ...
	  && (pageNo.pid == curPage.pid))
	{
	  ...
	  offset = getSlotOffset (slotNo);
	  Quadruple quadruple = new Quadruple(data, offset); // recLen is 32 fixed  
	  return quadruple;
	}
